...
*/
function printTransaction(_transactionData,_transactionCount){
  
  //Getting the relevant information from the transaction
  var transactionDetail = (` 
  Hash : ${_transactionData.hash}\n
  Nonce : ${_transactionData.nonce}\n
  From : ${_transactionData.from}\n     
  To : ${_transactionData.to}\n 
  Gas : ${_transactionData.gas}\n 
  GasPrice : ${_transactionData.gasPrice}\n 
  Input : ${_transactionData.input}\n `)
  
  //prints the transaction data
  printBoxedText(`Transaction #${_transactionCount}`,transactionDetail,"#FFFFFF","#FFFF00")
}
